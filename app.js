...
var PassbookInformation = mongoose.model(
	'PassbookInformation',
...
app.get('/shop', function (req, res) {
	Girvi.find({})
		.sort({ date: -1 })
		.exec(function (err, returnedData) {
			if (err) {
				console.log(err);
			} else {
...
app.get('/shop/:id', async function (req, res, next) {
...
app.get('/printReciept/:id', async (req, res, next) => {
	await Girvi.findById(req.params.id)
		.populate('passbookInformation')
		.exec(function (err, foundData) {
			if (err) {
...
app.post('/shop/:id/passbook/', function (req, res) {
	Girvi.findById(req.params.id, function (err, foundGirvi) {
		if (err) {
			...
			PassbookInformation.create(req.body.passbookInformation, function (
				err,
				returnedInfo
			) {
				if (err) {
